...
int color = 0;               // Color method
int type = 4;                // Fractal type
...
std::array<float, 2> c{};    // Value of C constant for Julia set

double d_zoom = 1. / 256;    // Zoom in the complex plane
...
int main()
{
    ...
    // Load shader
    if (!shader.loadFromFile("../shared/fractal.vert", "../shared/fractal.frag"))
    {
    ...
    while (window.isOpen())
    {
        ...
        ImGui::Combo("##type", &type, "Mandelbrot\0Julia\0Burning Ship\0Tricorn\0Newton 1\0Newton 2\0\0");
        ImGui::NewLine();
        ...
        if (type < 4) {
            ...
            ImGui::Combo("##color", &color, "Black and White\0Original\0Shades of Grey\0\0");
            ImGui::NewLine();
        }
